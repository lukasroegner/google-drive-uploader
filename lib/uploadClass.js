...
module.exports = class Upload {
	...
	async uploadChunk() {
		...
		let fileId = null,
			endedNormally = false, retryDelay = 2000;
		while (!endedNormally) {
			try {
				const res = await this.put(stream, chunkSize, `${chunkStart}-${chunkStart + chunkSize - 1}`);;
				fileId = res.fileId;
			...
			if (!endedNormally) {
				retryDelay += 1000;
				await new Promise(r => setTimeout(() => r(), Math.min(60 * 1000, retryDelay)));
			}
		}
